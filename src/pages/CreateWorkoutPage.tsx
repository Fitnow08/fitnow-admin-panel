...
export function CreateWorkoutPage() {
  ...
  const { data: categories } = useQuery({
    queryKey: ["train-categories"],
    queryFn: TrainService.getAllTrainCategory,
  });
  ...
  const create = useMutation({
    mutationFn: async (values: FormValues) => {
      // 1. создаём тренировку и получаем её id
      ...
      await TrainService.uploadTrainImage(train.id, values.image[0]);
      return train;
    },
    onSuccess: () => {
